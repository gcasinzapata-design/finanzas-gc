...
function getModel(systemPrompt) {
  ...
  // Use gemini-1.5-flash with v1 API (free tier: 1500 req/day)
...
function buildContext(tx, cards, debts, goals) {
  ...
  const byCat = {}
  tx.filter(t => t.type === 'gasto').forEach(t => {
  ...
  const todasDeudas = [
    ...cards.filter(c => c.current_balance > 0).map(c => ({
      nombre: `${c.bank} ${c.name}`, saldo: c.current_balance, tasa: c.tcea || 0, minimo: c.minimum_payment || 0
    })),
    ...debts.filter(d => d.current_balance > 0).map(d => ({
      nombre: d.name, saldo: d.current_balance, tasa: d.tea || 0, minimo: d.monthly_payment || 0
    }))
  ]
  const avalancha = [...todasDeudas].sort((a, b) => b.tasa - a.tasa)
  const bolaDeNieve = [...todasDeudas].sort((a, b) => a.saldo - b.saldo)

  return `SITUACION FINANCIERA REAL DEL USUARIO
...
Compromisos fijos (cuotas): S/ ${fmt(totalCompromisos)}/mes
Liquidez disponible: S/ ${fmt(liquidez)}
Ratio deuda/ingreso: ${ratioDeuda}%

TARJETAS DE CREDITO (${cards.length}):
${cards.map(c => `${c.bank} ${c.name}: Deuda S/${fmt(c.current_balance)} | Limite S/${c.credit_limit ? fmt(c.credit_limit) : 'N/A'} | Uso ${c.credit_limit ? (c.current_balance / c.credit_limit * 100).toFixed(0) : '?'}% | TCEA ${c.tcea || '?'}% | Pago min S/${fmt(c.minimum_payment || 0)} | Vence dia ${c.payment_due_date || '?'} | Corte dia ${c.cut_date || '?'}`).join('\n') || 'Sin tarjetas'}
TOTAL TARJETAS: S/ ${fmt(totalTarjetas)}

PRESTAMOS (${debts.length}):
${debts.map(d => `${d.name}${d.institution ? ` (${d.institution})` : ''}: Saldo S/${fmt(d.current_balance)} | Cuota S/${fmt(d.monthly_payment || 0)}/mes | TEA ${d.tea || '?'}% | ${d.remaining_installments || '?'} cuotas`).join('\n') || 'Sin prestamos'}
...
ORDEN AVALANCHA (mayor tasa primero - minimiza intereses):
${avalancha.map((d, i) => `${i + 1}. ${d.nombre}: S/${fmt(d.saldo)} al ${d.tasa}% - cuota min S/${fmt(d.minimo)}/mes`).join('\n') || 'Sin deudas'}

ORDEN BOLA DE NIEVE (menor saldo primero - mayor motivacion):
${bolaDeNieve.map((d, i) => `${i + 1}. ${d.nombre}: S/${fmt(d.saldo)} al ${d.tasa}%`).join('\n') || 'Sin deudas'}
...
export async function POST(req) {
  ...
  try {
    ...
    const systemPrompt = `Eres un asesor financiero personal experto para peruanos. Conoces a fondo el sistema financiero peruano: SBS, TCEA, TEA, AFP, CTS, gratificaciones, bancos locales.

${context}

INSTRUCCIONES:
- Da consejos ESPECIFICOS con NUMEROS EXACTOS basados en la situacion real del usuario
- Calcula Avalancha vs Bola de Nieve con matematica real
- Indica exactamente cuanto pagar en cada tarjeta/prestamo
- Detecta oportunidades de ahorro
- Habla en espanol peruano natural, directo
- Si faltan datos (ej. TCEA), pide que los complete en la seccion Tarjetas
- Usa S/ para soles`
    ...
    if (msg.includes('quota') || msg.includes('429')) return NextResponse.json({ reply: 'Limite de Gemini alcanzado. Intenta en unos minutos (limite gratuito: 15 req/min).' })
